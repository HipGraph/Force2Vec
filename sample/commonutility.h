...
template <class INDEXTYPE, class VALUETYPE>
void printCSRIND(CSR<INDEXTYPE, VALUETYPE> &A_csr, INDEXTYPE index){
        cout << "Vertex ID:" << index << endl;
        cout << "Rowptrs:" << A_csr.rowptr[index] << ", next:" << A_csr.rowptr[index+1];
        cout << endl;
        cout << "Colids:";
        for(INDEXTYPE i = A_csr.rowptr[index]; i < A_csr.rowptr[index+1]; i++){
                cout << A_csr.colids[i] << " ";
        }
        cout << endl;
}
