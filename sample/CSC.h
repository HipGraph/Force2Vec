...
public:
    ...
    CSC(IT mynnz, IT m, IT n, IT nt):nnz(mynnz),rows(m),cols(n) // costruttore di default 
    {
                            ...
                            IT* rowids_out, NT* values_out, IT* len_out)
{
    ...
    {
        IT idx=0;
        for(IT j=0, k=0; j<len_in && k < len_ri;)
        {
...
CSC<IT,NT> CSC<IT,NT>::SpRef2 (const IT* ri, const IT rilen, const IT* ci, const IT cilen)
{
    ...
    for(IT i=0; i< C.cols; ++i)         // combine step
    {
        IT cidx1 = colptr[ci[i]];
